...
function sanitizeBody(text: string, maxLen: number): string {
...
async function isAbuseHashBanned(hash: string): Promise<boolean> {
  ...
  .https.onCall(async (data, context) => {
    ...
    try {
      ...
      if (body.length > MAX_POST_LENGTH) {
        throw new functions.https.HttpsError(
          "invalid-argument",
          `Máximo ${MAX_POST_LENGTH} caracteres.`
        );
      }
      ...
      if (await isAbuseHashBanned(abuseHash)) {
        throw new functions.https.HttpsError(
          "permission-denied",
          "Tu cuenta no tiene permiso para publicar."
        );
      }
      ...
      throw new functions.https.HttpsError(
        "internal",
        "Error interno al crear publicación."
      );
    }
  });
...
export const createWallComment = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    try {
      ...
      console.log(
        `[WALL] New comment ${commentRef.id} on post ${postId}`
      );
      ...
      throw new functions.https.HttpsError(
        "internal",
        "Error interno al crear comentario."
      );
    }
  });
...
export const moderateContent = functions
  .region("us-central1")
  .https.onCall(async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
    ...
    try {
      ...
      throw new functions.https.HttpsError(
        "internal",
        "Error interno al moderar contenido."
      );
    }
  });
...
export const reportContent = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    try {
      ...
      throw new functions.https.HttpsError(
        "internal",
        "Error interno al reportar contenido."
      );
    }
  });
 ...
 */
export const blockWallAuthor = functions
  .runWith({secrets: ["WALL_ABUSE_SALT"]})
  .region("us-central1")
  .https.onCall(async (data, context) => {
    if (!context.auth) {
    ...
    try {
      ...
      if (!postId || (type !== "post" && type !== "comment")) {
        throw new functions.https.HttpsError(
          "invalid-argument",
          "Referencia de contenido inválida."
        );
      }
      ...
      const targetRef = type === "comment" ?
        postRef.collection("comments").doc(commentId as string) :
        postRef;
      ...
      const targetHash = targetDoc.data()?.abuseHash;
      if (typeof targetHash !== "string" || targetHash.length < 8) {
        throw new functions.https.HttpsError(
          "failed-precondition",
          "No se pudo identificar al autor."
        );
      }
      ...
      if (targetHash === ownHash) {
        throw new functions.https.HttpsError(
          "failed-precondition",
          "No puedes bloquear tus propias publicaciones."
        );
      }

      const alias = targetDoc.data()?.alias;
      await db
        .collection("users")
        .doc(uid)
...
export const banAbuseHash = functions
  .region("us-central1")
  .https.onCall(async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
    ...
    try {
      ...
      console.log(
        `[WALL] Hash ${abuseHash} banned. ${pendingPosts.size} posts rejected.`
      );
      ...
      throw new functions.https.HttpsError(
        "internal",
        "Error interno al banear usuario."
      );
    }
  });
