  ...
  Future<void> _startRealtimeSync(String uid) async {
    ...
    _realtimeSubscription = _favoritesDoc(uid).snapshots().listen(
      (doc) {
        ...
        final shouldKeepLocal = firstServerSnapshot && hadPendingLocalSave;
        firstServerSnapshot = false;
        if (!shouldKeepLocal && _pendingCloudSave) {
        ...
        if (doc.exists) {
          ...
          final cloudFavorites = versesList
              ...
              .toList();
          _cache = shouldKeepLocal ? localFavorites : cloudFavorites;
        } else {
          if (shouldKeepLocal) {
    ...
    await firstSnapshot.future.timeout(
      ...
      onTimeout: () =>
          debugPrint('⭐ [FAV_REPO] Initial realtime snapshot timeout'),
    );
  }
