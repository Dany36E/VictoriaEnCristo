...
class ProgressRepository {
  ...
  Future<void> _doConnectUser(String uid, List<String> selectedGiants, double threshold) async {
    ...
    try {
      ...
      final pendingDays = <String, VictoryDay>{};
      for (final dateISO in _pendingWrites) {
        final day = _cache[dateISO];
        if (day != null) pendingDays[dateISO] = day;
      }
  ...
  Map<String, int> getDayGiantStates(DateTime date, List<String> selectedGiants) {
    ...
    return {for (final giant in selectedGiants) giant: 0};
  }
  ...
  Future<void> _saveToCloud(String uid, VictoryDay day) async {
    final saved = await _trySaveToCloud(uid, day);
    if (!saved) _addPendingWrite(day.dateISO);
  }

  Future<bool> _trySaveToCloud(String uid, VictoryDay day) async {
    try {
      await _firestore
          .collection('users')
          .doc(uid)
  ...
  void _startRealtimeSync(String uid) {
    // Filtramos por últimos ~365 días (suficiente para racha y stats anuales).
    ...
    _realtimeSubscription = _firestore
        ...
        .doc(uid)
        .collection('victoryDays')
        .where(FieldPath.documentId, isGreaterThanOrEqualTo: cutoffISO)
        .snapshots()
        .listen(
          (snapshot) {
  ...
  Future<void> _replayPendingWrites(String uid, Map<String, VictoryDay> pendingDays) async {
    ...
    debugPrint(
      '📊 [PROGRESS_REPO] Pending replay: ${pendingDays.length - failedWrites.length}/${pendingDays.length} applied',
    );
  }
