...
import 'learning/learning_registry.dart';
...
class AccountSessionManager {
  ...
  bool _connectivityRetryAttached = false;
  bool _repositoryHydrationListenersAttached = false;
  Timer? _localHydrationDebounce;
  List<String> _currentSelectedGiants = const ['general'];
  double _currentThreshold = VictoryScoringService.defaultThreshold;
  ...
  Future<void> _purgeAllLocalCache() async {
    try {
      ...
      // Keys de preferencias/progreso que ahora tienen sync propio.
      await UserPrefCloudSyncService.I.clearLocalCache();
      await LearningCloudSync.I.clearLocalCache();
      await prefs.remove('learning.talents.v1');
      await prefs.remove('learning.talents.ledger.v1');

      // Keys de otros servicios
      await prefs.remove('cloud_migration_done');

      for (final key in prefs.getKeys().toList(growable: false)) {
        if (key.startsWith('bible_user_data_cache_v1.') ||
            key.startsWith('chapter_notes_cache_v1.') ||
            key.startsWith('verse_collections_cache_v1.')) {
  ...
  Future<UserProfile?> _bootstrapNewUser(String uid) async {
    ...
    try {
      ...
      if (profile == null) {
        ...
        await DailyPracticeService.I.init();
        await LearningRegistry.I.syncForCurrentUser();
      ...
      _currentSelectedGiants = List<String>.from(selectedGiants);
      _currentThreshold = threshold;
      _attachRepositoryHydrationListeners();
      ...
      // 9d. Asegurar progreso de juegos/aprendizaje si el registry arranco
      // antes de que FirebaseAuth tuviera usuario, o si hubo cambio de cuenta.
      await LearningRegistry.I.syncForCurrentUser();

      // 9e. Re-registrar el token FCM bajo el nuevo uid. Esto es idempotente
      ...
      // 11. Inicializar conectividad y reintentar sync pendiente al reconectar
      unawaited(_initConnectivityRetry());
  ...
  Future<void> _initConnectivityRetry() async {
    await ConnectivityService.I.init();
    if (_connectivityRetryAttached) return;

    _connectivityRetryAttached = true;
    ConnectivityService.I.isOnline.addListener(_onConnectivityChangedForSync);
  ...
  Future<void> _retryPendingCloudSaves() async {
    if (_currentSessionUid == null) return;

    try {
      await Future.wait([
        ProgressRepository.I.retryPendingWrites(),
        JournalRepository.I.retryPendingOps(),
        PlansRepository.I.retryPendingWrites(),
        FavoritesRepository.I.retryPendingCloudSave(),
        BadgeRepository.I.retryPendingCloudSave(),
  ...
  void _attachRepositoryHydrationListeners() {
  ...
  void _detachRepositoryHydrationListeners() {
    ...
    _localHydrationDebounce = null;
    _repositoryHydrationListenersAttached = false;
    ProgressRepository.I.onCloudCacheChanged = null;
    JournalRepository.I.onCloudCacheChanged = null;
    PlansRepository.I.onCloudCacheChanged = null;
    FavoritesRepository.I.onCloudCacheChanged = null;
    BadgeRepository.I.onCloudCacheChanged = null;
  }

  void _scheduleLocalHydrationFromRepositories() {
  ...
  Future<void> _hydrateLocalServicesFromCloud(List<String> selectedGiants, double threshold) async {
    try {
      ...
      final localEntries = cachedEntries.values
          .map(
            (cloud) => JournalEntry(
              id: cloud.id,
