...
class FeedbackEngine {
  ...
  Future<void> init() async {
    ...
    // Desktop guard: just_audio not available on Windows/Linux. Keep haptic
    // feedback (handled separately) but skip SFX player creation.
    if (!PlatformCapabilities.supportsAudioPlayback) {
      _log('INIT', 'SFX disabled on this platform (${PlatformCapabilities.currentLabel})');
  ...
  void _trigger(FeedbackEvent event, Future<void> Function() hapticFn) {
    ...
    if (hapticsEnabled.value) {
      ...
      _log(
        'HAPTIC',
        '📳 ${event.name} -> ${hapticFn == HapticFeedback.lightImpact ? 'lightImpact' : 'mediumImpact'}',
      );
    }
  ...
  Future<void> _playSfx(FeedbackEvent event) async {
    ...
    try {
      ...
      _log(
        'SFX',
        '🎵 ${event.name} asset=$asset vol=${finalVolume.toStringAsFixed(2)} (base=${eventVolume.toStringAsFixed(2)})',
      );
    } catch (e) {
